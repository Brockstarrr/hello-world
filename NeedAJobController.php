...
class NeedAJobController extends Controller 
{
	...
	{
		$Data['Title'] 				= 'Need A Job List';
		$Data['Menu'] 				= 'CMS';
		$Data['SubMenu'] 			= 'NeedAJob';
		return View('Admin/NeedAJob/List')->with($Data);
	}
	public function Add(){
		$Data['Title'] 				= 'Add Need A Job';
		$Data['Menu'] 				= 'CMS';
		$Data['SubMenu'] 			= 'NeedAJob';
		return View('Admin/NeedAJob/Add')->with($Data);
	}
	public function EditNeedAJob($ID){
		$id = base64_decode($ID);
		...
		$Data['Result'] 	= $Result;
		$Data['Title'] 		= 'Edit Need A Job';
		$Data['Menu'] 		= 'CMS';
		$Data['SubMenu'] 	= 'NeedAJob';
		return View('Admin/NeedAJob/Edit')->with($Data);
	}
	public function InsertNeedAJob(Request $request){
		$Data = $request->all();
      ...
      $Save['image'] 		= $ImageName;
		...
		$msg = Common::AlertErrorMsg('Success','Need A Job Details Has been Saved.');
		Session::flash('message', $msg);
		return Redirect()->back();
	}
	public function SaveNeedAJob(Request $request){
		$Data = $request->all();
	...
	public function NeedAJobListing(Request $request){
		$Data = $request->all();
      ...
      if($row->image!=''){
        ...
        <td>
        	...
        	<a href="<?=route('EditNeedAJob',array('ID'=>base64_encode($row->id)) )?>">
	...
	public function NeedAJobChangeStatus(Request $request){
		$Data 	= $request->all();
		$id 		= $Data['id'];
		$status = $Data['status'];
